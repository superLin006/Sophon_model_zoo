...
void Usage() {
  printf("Usage:\n"
         ...
         "  -s, --system    : Set system prompt (default: \"You are a helpful assistant.\")\n"
...
void processArguments(int argc, char *argv[], std::string &model_path,
                      ...
                      bool &enable_history, std::string &prompt,
  ...
  while ((option = getopt_long(argc, argv, "m:c:d:p:s:eh", longOptions,
                               &optionIndex)) != -1) {
    switch (option) {
    case 'm':
      model_path = optarg;
...
int main(int argc, char **argv) {
  ...
  processArguments(argc, argv, model_path, config_path, devices,
                   enable_history, prompt, sys_prompt);
  if (model_path.empty()) {
    Usage();
    exit(EXIT_FAILURE);
  }
  ...
  model.init(model_path, config_path, sys_prompt, enable_history, devices);
