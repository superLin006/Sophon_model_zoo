...
static void print_usage(const char* prog) {
    std::cerr << "Usage: " << prog << " [options]\n"
              << "  --model-dir  <dir>   directory containing bmodels and assets (default: ../models)\n"
              ...
              << "  --temp       <val>   temperature (default: 0.0001)\n"
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[2/3] Inferring: \"" << text.substr(0, 60)
              << (text.size() > 60 ? "..." : "") << "\"\n";
    ...
    std::vector<float> pcm = tts.infer(text, params);
