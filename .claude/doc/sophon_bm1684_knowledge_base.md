...
## 七、量化精度限制（Qwen3-TTS 2026-08 铁证结论）

### 核心结论

**TPU 量化网络与 CPU 同 dtype 模拟的精度不同**：CPU torch 模型（eager）层内是 fp32 计算、仅层间激活量化；TPU 量化网络（F16/BF16/W8BF16）**每个算子的中间激活都存回量化 dtype**（bf16 7 位 / f16 10 位尾数）。对生成式模型（TTS code_predictor、LLM decode 等嵌套自回归），误差经 argmax 翻转即进入错误轨迹（code 循环、不 EOS）。

### 判断模型能否量化的经验法则

| 模型输出类型 | 量化档位建议 | 依据 |
|------------|------------|------|
| 离散分类/ASR token | W8BF16 通常可用 | 单次 argmax 容错大（Qwen3-ASR/HY-MT 已验证） |
| 连续 hidden 驱动的嵌套自回归（TTS CP） | **必须 F32** | 5 层 × 15 步 × 数百帧误差累积，量化必退化 |
| 大模型 talker/decoder | W8BF16 多数可用、个别长句退化 | 测试多句确认；退化概率与句子长度相关 |

### 验证流程（防误判）

1. **CPU 同 dtype 闭环模拟先行**：`from_pretrained(..., dtype=torch.bfloat16, attn_implementation="eager")` 端到端跑完整输入，与 fp32 参考对比
   - CPU 模拟通过 → 模型本身可量化，板卡退化是 TPU 实现精度限制
   - CPU 模拟也错 → 量化误差本身过大，与该档位无关
2. **板卡验证必须端到端闭环 + 多句**：单帧/短输入 codes16 全对不能证明可用（Qwen3-TTS 的 F16 12 帧全对但长句退化）；至少 3-5 条不同长度输入，全部自然 EOS 才算通过
3. **`--high_precision` 不是万能药**：norm 强制 fp32 对部分模型是负优化（数值路径改变引入新误差），必须实测对比

### 已证实的案例（Qwen3-TTS）

- CP（code_predictor）：F16/BF16/W8BF16/混合精度/HP 全部长句退化 → **必须 F32**
- talker（28 层）：W8BF16 可用（4 句测试 1 句长句退化，其余正常）→ 接受小概率退化换 30% 提速
- CPU bf16 闭环对照：完全正常 → 证明是 TPU 实现限制，非 bug、非模型不可量化
