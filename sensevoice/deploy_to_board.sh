...
usage() {
    printf '用法: BOARD_IP=<ip> [BOARD_USER=root] [BOARD_PORT=22] [BOARD_DIR=/data/sensevoice] %s [F16|F32] [--test]\n' "$0"
}
...
BOARD_DIR="${BOARD_DIR:-/data/sensevoice}"
MODEL_DIR="${MODEL_DIR:-${SCRIPT_DIR}/models/BM1684X}"
BINARY="${BINARY:-${SCRIPT_DIR}/cpp/build/sensevoice_bm1684}"
...
BMODEL="${MODEL_DIR}/sensevoice_small_${PRECISION}.bmodel"
require_file "$BINARY"
require_file "$BMODEL"
...
"${SCP[@]}" "$BMODEL" "${MODEL_DIR}/tokens.txt" "${REMOTE}:${BOARD_DIR}/models/"
...
check_md5 "$BMODEL" "${BOARD_DIR}/models/$(basename "$BMODEL")"
check_md5 "${MODEL_DIR}/tokens.txt" "${BOARD_DIR}/models/tokens.txt"
